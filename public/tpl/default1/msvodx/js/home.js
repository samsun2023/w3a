...
var MAC={
    ...
    'Fav': function(u,s){
        try{ window.external.addFavorite(u, s);}
            ...
            try{window.sidebar.addPanel(s, u, "");}catch (e){ alert(lang('CopyFail')); }
        }
    },
    'Open': function(u,w,h){
    ...
    'Verify': {
        ...
        'Show':function(){
            return '<img class="mac_verify_img" src="'+ maccms.path +'/index.php/verify/index.html?" title="'+lang('ChangeImg')+'">';
        }
    },
    'PageGo':{
        'Init':function() {
    ...
    'Score': {
        'Init':function(){
            ...
            $.ajax({
                ...
                timeout: 5000,
                error: function(){
                    $(".mac_score").html(lang('LoaderFail'));
                },
                success: function($r){
                    MAC.Score.View($r);
                }
            ...
            $('.mac_star').raty({
                ...
                click: function(score, evt) {
                    $.ajax({
                        ...
                        dataType:'json',
                        error: function(){
                            $('.star_box').attr('title', lang('Network'));
                        },
                        success: function(json){
                            if(json.status == 1){
    ...
    'Gbook':{
        ...
        'Show':function($page){
            $.ajax({
                ...
                timeout: 3000,
                error: function(){
                    $(".mac_gbook_box").html(lang('ComFail'));
                },
    ...
    'Pop':{
        'Show':function($modalid,$url,$callback) {          
            $.ajax({
                ...
                timeout: 3000,
                error: function(){
                   $('body').append(alert(lang('PayVideo11')));
                },
                success:function($r){
                    $('body').append($r);
                    $($modalid).modal('show');
        ...
        'Init':function(){
            ...
            $('body').on('click', '.comment_reply', function(e){
                ...
                if($that.attr("data-id")){
                    ...
                    if (str == lang('ReplyFail')) {
                        $that.html(lang('Reply'));
                        return false;
                    }
                    if (str == lang('Reply')) {
                        $('.comment_reply').html(lang('Reply'));
                    }
