    ...
    $.get('/api/rewardranking.html',{nums:5},function(e){
     ...
     if(e.resultCode==0){
            e.data.forEach(function (t) {
                ...
                html += '<p class="name">'+t.nickname+'</p> <span>'+lang('GiveGold')+'：'+t.sums+'</span> <p class="place"><strong>'+lang('GiveSort')+'：</strong>NO.'+t.no+'</p></li>';
            })
           $(".ranking").find('ul').append(html);
        }else if(e.resultCode==4003){
         html='<a class="isnull">'+lang('NoGiveData')+'</a>'
         $(".ranking").append(html);
        }
   },'json');
});