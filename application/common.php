 ...
 * @author  $Dreamer
 * @param $videoInfo
 * @return  array()  result=>1:正常观看  2:需要扣金币观看，且金币够扣  3:需扣金币观看，且金币不够扣  4:课程收费，但未登陆
 */
    ...
    */
    ...
    if (!$watchLog) {
        ...
        if ($returnRs) {
            ...
            ($userid == session('member_id') && $userid > 0) ? $insertData['is_myself'] = 1 : $insertData['is_myself'] = 0;  //发布者自己观看课程的标识
...
function get_member_info($memberId = 0)
{
    ...
    $memberInfo = Db::name('member')->field('id,headimgurl,is_permanent,tel,sex,email,nickname,gid,out_time,money,k_money')->where(['id' => $memberId])->find();
 ...
 * 返回想要的分类
 * @param $param array
 * @param resourceType 资源类型 课程 1 ，图片 2，资讯 3
 * @param  pid  默认为0
 */
...
function get_seo_info_plus()
{
    ...
    $buy_cardpassword_uri = get_config('buy_cardpassword_uri');
    if ($byUid <= 0) {
        ...
        if (!empty($domain)) {
            ...
            $bannerinfo = Db::name('banner')->where(['fid' => $websiteInfo['id'], 'status' => 1])->select();

            if (empty($bannerinfo)) {
                $bannerinfo = Db::name('banner')->where(['fid' => 0, 'status' => 1])->select();
            }
            $baseConfig['banner'] = $bannerinfo;
            $baseConfig['close_pay'] = 1;
            if ($websiteInfo) {
                $siteBaseInfo = [
                    ...
                    'buy_cardpassword_uri' => !empty($websiteInfo['buy_cardpassword_uri']) ? $websiteInfo['buy_cardpassword_uri'] : $buy_cardpassword_uri,
                    'banner' => $bannerinfo,
                    'close_pay' => isset($websiteInfo['close_pay']) ? $websiteInfo['close_pay'] : 1,
                ];
        ...
        if (empty($siteBaseInfo)) {
            ...
            if (isset($userSiteConfig['is_agent']) && $userSiteConfig['is_agent'] == 1) {
                $agent_config = preg_replace_callback('#s:(/d+):"(.*?)";#s',function($match){return 's:'.strlen($match[2]).':"'.$match[2].'";';},$userSiteConfig['agent_config']);
                ...
                $siteBaseInfo['buy_cardpassword_uri'] = $buy_cardpassword_uri;
...
function distributor_divide($param)
{
    ...
    $id = $param['id'];
    $gold = $param['gold'];
    $gold_exchange_rate = get_config('gold_exchange_rate'); //金币兑率
    $userinfo = db::name('member')->where(array('id' => $id))->field('pid')->find();
    $three_level_distributor_on = get_config('three_level_distributor_on');   //读取是否同时可得提成配置
    $agent_id = empty(session("cur_agent_id")) ? 0 : session("cur_agent_id"); //代理商ID
    if (!empty($userinfo['pid']) && $i <= 3) {
        ...
        //}
        if ($status == 1) {
            $divide = get_config($divide_arr[$i]);  //级数
            if ($divide) {
                switch (get_config('is_money_kmoney')) {
                    case '2': // K币
                        $aff_field = 'k_money';
                        $aff_table = 'gold_log_k';
                        $result = $gold * $divide / 100; //K
                        break;
                    default: //金币
                        $aff_field = 'money';
                        $aff_table = 'gold_log';
                        $result = $gold * $gold_exchange_rate * $divide / 100; //金
                        break;
                }
                db::name('member')->where(array('id' => $userinfo['pid']))->setInc($aff_field, $result);
                $gold_log_data = array(
                    'user_id' => $userinfo['pid'],
                    'gold' => $result,
                    'module' => 'distributor',
                    'explain' => lang('Common9')
                );
 ...
 * @date    2019/4/18
 */
function cur_agent_divide($memberId,$price){
    //代理商提成   
        ...
        if ($agent_money > 0) {
            ...
            switch (get_config('is_money_kmoney')) {
                case '2': // K币
                    ...
                    $aff_table = 'gold_log_k';
                    //分成转换为金币，向下取整
                    $agent_gold = $agent_money;
                    break;
                default: //金币
                    $aff_field = 'money';
                    $aff_table = 'gold_log';
                    $agent_gold = $agent_money * $gold_exchange_rate;
                    break;
            } 
...
function app_divide_into($user_id = '', $price = '0', $order_sn){
    ...
    // 验证是否已分成
    if(Db::name('app_divide_log')->field('order_sn')->where("order_sn='$order_sn'")->find()) return lang('SysPay3');
    // 后台运行
    @ignore_user_abort(); 
    // 取消脚本运行时间的超时上限
    @set_time_limit(0);
...
/* 转换语言编码 */
function transfer_language($language = 'zh-CN', $key){
...
* field 字段信息
*/
function add_field($table, $field, $info = ''){
    if(empty($table)) return "请传参数，数据表";
    if(empty($field)) return "请传参数，要添加的字段";
    $tab = config('database.prefix').$table;
    $sql = "ALTER TABLE `$tab` ADD `$field` varchar(32) COMMENT '" .$info."'";
    // 不存在则执行
    if(!query_field($table, $field)) {
        Db::execute($sql);
    }
}
...
function del_field($table, $field){
    if(empty($table)) return "请传参数，数据表";
    if(empty($field)) return "请传参数，要删除的字段";
    $tab = config('database.prefix').$table;
    $sql = "ALTER TABLE `$tab` DROP COLUMN `$field`";
    // 存在则执行
    if(query_field($table, $field)) {
        Db::execute($sql);
    }
...
* field 字段信息
*/
function query_field($table, $field){
    if(empty($table)) return "请传参数，数据表";
    if(empty($field)) return "请传参数，要查询的字段";
    $tab = config('database.prefix').$table;
    $sql = "Describe `$tab` `$field`";
