        ...
        foreach ($list_data as $k=>$v){
            ...
            switch ($v['type'])
            {
                case 1:
                    $db = Db::name('video');
                    $where['pid'] = 0;
                    $field = 'id,title,click,good,thumbnail,play_time,gold,update_time,reco,type';
                ...
                case 3:
                    $db = Db::name('novel');
                   $field = 'id,title,click,good,update_time,gold,update_time,thumbnail,short_info,tag';
                   $list = $db->where($where)->field($field)->order('update_time desc')->limit($resourceLimit)->select();
                  foreach ($list as $key =>$vo){
                      $list[$key]['tag'] = explode(",",$vo['tag']);
                  }
                   $v['list'] = $list;
                   $novel_list[] = $v;
                   break;
