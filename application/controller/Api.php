...
class Api extends Controller
{
    ...
    public function __construct(Request $request)
    {
        ...
        $noAuthAct = ['getatlas', 'getcaptcha', 'givegood',
            ...
            'is_login', 'rewardranking', 'getthemeinfos', 'createqrcode','getversion'
        ];
...
//                    邮箱发送验证码处理
        ...
        switch ($type) {
            case 1:
                ...
                if (empty($result_video)) {
                    die(json_encode(['resultCode' => 4005, 'error' => '当前课程不存在']));
                }
                //判断课程是否已经收藏
                $result_collect = $collect_db->where(['user_id' => $member_id, 'video_id' => $id])->find();
                if ($result_collect) {
                    die(json_encode(['resultCode' => 4005, 'error' => '该课程已经收藏过了']));
                }
                //插入用户收藏日志
                $collect_data = [
                    'user_id' => $member_id,
    ...
    /**
     * 评论接口
     * @param resourceType  评论资料对象类型： 2/3/1  image / novel / video  图片/资讯/课程
    ...
    function comment(Request $request)
    {
        ...
        $to_user = $request->post('to_user/s', 0);
        $to_id = $request->post('to_id/s', 0);
        $insertData = [
            'add_time' => time(),
            'last_time' => time(),
    ...
    function getCommentList(Request $request)
    {
        ...
        $where = "status = 1 and  resources_type = $resourceType and resources_id = $resourceId and to_id = 0";
    ...
    function payVideo(Request $request)
    {
        ...
        if (!$videoInfo) die(json_encode(['resultCode' => 4005, 'error' => '课程资源不存在.']));
        ...
        if (isset($authRs['result'])) {
            //result=>1:正常观看  2:需要扣金币观看，且金币够扣  3:需扣金币观看，且金币不够扣  4:课程收费，但未登陆
            $returnData = [];
            switch ($authRs['result']) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $returnData = ['resultCode' => 0, 'message' => '课程需要付费,请登陆后观看', 'data' => ['rs' => 4, 'videoInfo' => $videoInfo]];
                    break;
            }
            //TODO 防止IOS+UC试看问题,无法识别UC浏览器
            /***课程地址加截部分课程资源（防止IOS+UC试看问题）******************$dreamer 2018/1/9****************************/
...
//                   手机发送验证码处理
    ...
    {
        $type  = $request->post('type') ? $request->post('type') : ''; //路由  k 为K币
        $limit = $request->post('limit/d', 8);
        ...
        $login_status = check_is_login();
        if ($login_status['resultCode'] != 1) die(json_encode(['resultCode' => 4005, 'error' => $login_status['error']]));
        $count = Db::name($table)->where(['user_id' => $user_id])->order('add_time', 'desc')->count();
        $data['count'] = $count;
        $start = ($limit * $page) + $offset;
        $data_list = Db::name($table)->where(['user_id' => $user_id])->order('add_time', 'desc')->limit($start, $limit)->select();
        if (!empty($data_list)) {
            $data['list'] = $data_list;
            die(json_encode(['resultCode' => 0, 'data' => $data, 'message' => '加载完成']));
        } else {
    ...
    public function moreMember(Request $request)
    {
        $offset = $request->post('offset/d', '0');
        $limit = $request->post('limit/d', 8);
        $page = $request->post('page/d', 0);
        $key_word = $request->post('key_word/s', '');
        $order = "add_time asc";
        $where="status = 1 and nickname like '%$key_word%'";
        $count = Db::name('member')->where($where)->count();
        $start = ($limit * $page) + $offset;
        $end = $start + $limit;
        $list = Db::name('member')->where($where)->order($order)->limit($start, $limit)->select();
        $data['count'] = $count;
        $data['isMore'] = ($count > $end) ? 1 : 0;
        if (!empty($list)) {
            $data['list'] = $list;
    ...
    {
        $offset = $request->post('offset/d', '20');
        $atlas_id = $request->post('atlas_id/d', '0');
        $limit = $request->post('limit/d', 8);
        $page = $request->post('page/d', 0);
        $order = empty($order) ? 'sort asc,add_time desc' : $order;
        $where="status = 1 and atlas_id = $atlas_id";
        $count = Db::name('image')->where($where)->count();
        $start = ($limit * $page) + $offset;
        $end = $start + $limit;
        $list = Db::name('image')->where($where)->limit($start, $limit)->select();
        $data['count'] = $count;
        $data['isMore'] = ($count > $end) ? 1 : 0;
        if (!empty($list)) {
            $data['list'] = $list;
    ...
    public function moreData(Request $request)
    {
        ...
        $limit = $request->post('limit/d', 8);
        $page = $request->post('page/d', 0);
        $resources_type = ($type == 'video') ?  1 : (($type == 'atlas') ? 2 : 3);
        $field = array(
        ...
        $user_id=$request->post('userid/d',0);

        $orderCode = empty($request->post('orderCode')) ? 'lastTime' : $request->post('orderCode');
        switch ($orderCode) {
            case 'lastTime':
                $order = ($type == 'image') ? "{$pre}{$type}.add_time,{$pre}{$type}.id desc" : "{$pre}{$type}.update_time desc";
                break;
            case 'lastTimeASC':
                $order = "{$pre}{$type}.update_time asc";
                break;
            case 'hot':
                $order = "{$pre}{$type}.click desc";
                break;
            case 'hotASC':
                $order = "{$pre}{$type}.click asc";
                break;
            case 'reco':
                $order = "{$pre}{$type}.reco desc";
                break;
            case 'recoASC':
                $order = "{$pre}{$type}.reco asc";
                break;
            default:
                $order = "{$pre}{$type}.update_time desc";
                break;
        }
        $order = empty($order) ? 'id desc' : $order;
        if ($type == 'video') {
            $where = "{$pre}{$type}.status = 1 and {$pre}{$type}.is_check=1  and {$pre}{$type}.pid = 0 ";
        } else {
            $where = "{$pre}{$type}.status = 1 and {$pre}{$type}.is_check=1" ;
        }
        if (!empty($wheres)) {
            $where .= str_replace(array('select','insert','delete','alter','*','--',';'),'',$wheres); //防注入 $DM 2018/04/18
            //$where.=" and title like '%".$request->param('keyword')."%'";
        }
        if (!empty($tag_id)) {
            $where .= " and FIND_IN_SET( $tag_id, {$pre}{$type}.tag)";
        }

        if($user_id>0) $where.=" AND {$pre}{$type}.user_id=$user_id";

        if (!empty($cid)) {
            $class_sublist = Db::name("class")->where(array("status" => 1, "pid" => $cid))->field("id,name")->select();
            if (empty($sub_cid)) {
                if (empty($class_sublist)) {
                    $where .= " and {$pre}{$type}.class = $cid";
                } else {
                    $param = array(
                        'db' => 'class',
                    ...
                    $where .= " and ({$pre}{$type}.class = $cid or {$pre}{$type}.class in (" . implode(',', $sub_array) . "))";
                }
            } else {
                $where .= " and {$pre}{$type}.class = $sub_cid";
            }
        ...
        if (in_array($orderCode, ['pay'])) {
            $list = Db::view("{$pre}{$type}", "$field[$type]")
                ->view("{$type}_watch_log", ["{$type}_id","count({$type}_watch_log.id)"=>'num'], "{$pre}{$type}.id={$type}_watch_log.{$type}_id and {$type}_watch_log.gold > 0", 'LEFT')
                ->group("{$pre}{$type}.id")->order('num', 'DESC')
                ->where($where)
                ->limit($start, $limit)
                ->select();
        }elseif (in_array($orderCode, ['discuss'])) {//评论数
            $list = Db::view("{$pre}{$type}", "$field[$type]")
                ->view('comment', ['resources_id',"count(comment.id)"=>'num'], "{$pre}{$type}.id=comment.resources_id and comment.resources_type = {$resources_type}", 'LEFT')
                ->group("{$pre}{$type}.id")->order('num', 'DESC')
                ->where($where)
                ->limit($start, $limit)
                ->select();
    ...
    public function get_pay(Request $request)
    {
        ...
        $tx_type = $request->post('tx_type');  //提现类型  金币 / K币
        //判断管理员是否开启了用户提现功能
        ...
        if (intval($gold) < intval($min_withdrawals)) {
            die(json_encode(['statusCode' => 4005, 'message' => "提现失败，最低提现数为：".intval($min_withdrawals)]));
        }
        //判断用户提现帐户是否存在
        $money_account = Db::name('draw_money_account')->where(['id' => $type, 'user_id' => $memberId])->find();
        if (empty($money_account)) {
            die(json_encode(['statusCode' => 4005, 'message' => "你的收款帐户不存在！"]));
        }
        //提现类型 k为K币，J为金币
        switch ($tx_type) {
            case 'k': //K币
                if (intval($menber_info['k_money']) < intval($gold)) {
                    die(json_encode(['statusCode' => 4005, 'message' => "提现失败：你的账户K币不足"]));
                }
                $money_field = 'k_money';
                $money = $gold;
                $explain = 'K 币提现';
        ...
        //验证通过，写入数据库,更新用户金币
        $result = Db::name('member')->where(['id' => $memberId])->setDec($money_field, $gold);
        //插入成功后
        if ($result) {
            if($tx_type == 'k'){
                $gold_log_data = array(
            ...
            write_gold_log($gold_log_data,$table);
            //写入提现记录
            $data = array(
                'user_id' => $memberId,
                'gold' => $gold,
            ...
            if ($insert_info) die(json_encode(['statusCode' => 0, 'message' => "你的提现申请已提交成功"]));
        }
    ...
    function pay_way(Request $request)
    {
        ...
        if ($request->isAjax()) {
            ...
            if ($type == 'Bank') {
                ...
                $account_name = trim($request->post('account_name/s'));
                $img = trim($request->post('img/s'));
                if (empty($alipayaccount)) $msg = '请填写必要的选项后再试';
                if (empty($account_name)) $msg = '请填写真实姓名';
                if (empty($img)) $msg = '请填写必要的选项后再试';
    ...
    function use_card_password(Request $request)
    {
        ...
        //$cur_agent_id = empty(session("cur_agent_id")) ? 0 : session("cur_agent_id"); //代理ID
        /*if ($cur_agent_id > 0 && $memberId != $cur_agent_id) {
            ...
            $gold_exchange_rate = get_config('gold_exchange_rate'); //金币兑换比例
            //分成转换为金币，向下取整
            $agent_gold = $info['price'] * $agent_commission;
            //$agent_gold = (int)$agent_money;
            //为代理商增加分成金币
            if ($agent_gold > 0) {
                //代理提成入库
                switch (get_config('is_money_kmoney')) {
                    case '2': // K币
                        $aff_field = 'k_money';
                        $aff_table = 'gold_log_k';
                        break;
                    default: //金币
                        $aff_field = 'money';
                        $aff_table = 'gold_log';
                ...
                Db::name('member')->where(array('id'=>$cur_agent_id))->setInc($aff_field, $agent_gold);
                //提成写入记录
                $gold_log_data = array(
                    'user_id' => $cur_agent_id,
                    'gold' => intval($agent_gold),
                    'module' => 'Order',
                    'explain' => '代理分成'
                );
                write_gold_log($gold_log_data,$aff_table);
            }
        }*/
    ...
    function addResourceFromGather(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'video':
                ...
                $data = [
                    'title' => !empty(trim($request->post('title'))) ? trim($request->post('title')) : exit('错误:课程标题不能为空')//标题
                    , 'info' => $request->post('info')//说明
                    , 'short_info' => $request->post('short_info')//短说明
                    , 'key_word' => $request->post('key_word')//关键词
                    , 'url' => !empty(trim($request->post('url'))) ? trim($request->post('url')) : exit('错误:课程播放地址不能为空')//课程播放地址 Mp4/m3u8
                    , 'download_url' => $request->post('download_url')//课程下载地址
