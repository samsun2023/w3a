...
if(is_wechat() && $logincode=='wechat'){
    ...
    public function wechat_callback($log_state){
        ...
        $unionid = $arr['unionid'];
        //先判断用户是否存在
        $userinfo = Db::name('member')->where(['unionid'=>$unionid])->find();
        if(empty($userinfo)){
            $userinfo = Db::name('member')->where(['openid'=>$openid])->find();
            if($userinfo){
                Db::name('member')->where(['openid'=>$openid])->update(['unionid'=>$unionid]);
            }
        }
        //用户信息入库
        if(!empty($arr)&&empty($userinfo)){
            ...
            Db::name('member')->where(['unionid'=>$unionid])->update(['access_token'=>$token]);
            //写入session
