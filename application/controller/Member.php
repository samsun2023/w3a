...
class Member extends BaseController
{
    ...
    public function __construct(Request $request)
    {
        ...
        if (!in_array(request()->action(),$noCheckAct)) {
            ...
            if(empty($user_info)){
                if($request->isMobile())  $this->error('该用户已在其他地方登录', url('Index/login'), null, 3);
                $this->error('该用户已在其他地方登录', url('Index/index'), null, 3);
            }
        }
    ...
    public function collection_video(Request $request)
    {
        ...
        $this->assign('page_title', '课程收藏');
        $this->assign('navTopTitle', '课程收藏');
    ...
    public function video(Request $request)
    {
        ...
        $field = 'id,title,click,thumbnail,update_time,play_time,add_time,gold,is_check,status,hint';
        $list = $db->where($where)->field($field)->order($order)->paginate(10);
        $pages = $list->render();
        $this->assign('list', $list);
        $this->assign('pages', $pages);
        $this->assign('user_id', $this->member_id);
        $this->assign('page_title', '我的课程');
        $this->assign('navTopTitle', '我的课程');
    ...
    public function addVideo(Request $request)
    {
        ...
        $upload_permissions = get_config('upload_permissions');
        if($upload_permissions == 2){
            $memberInfo = get_member_info($this->member_id);
            if($memberInfo['is_permanent'] != 1 && $memberInfo['out_time'] < time()){
                layerJump('只有会员才能上传', 2, 2);exit;
            }
        }
        ...
        if ($this->request->isPost()) {
            ...
            $message = [
                ...
                'class.require' => '请选择课程分类',
                'url.require' => '请选择课程地址不为空'
            ];
            ...
            if($data['is_check'] == 1){
                $reward = get_config('upload_video_reward');
                if(!empty($reward)){
                    db::name('member')->where(array('id'=>$this->member_id))->setInc('money',$reward);
                }
            }
            ...
            $this->assign('page_title', '添加课程');
            $this->assign('navTopTitle', '添加课程');
            return view();
        }
    }
    ...
    public function editVideo(Request $request)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $this->assign('tag_list', $tag_list);
            $this->assign('class_list', $class_list);
            $this->assign('page_title', '编辑课程');
            $this->assign('navTopTitle', '编辑课程');
            return view();
        }
    }
    ...
    public function novel(Request $request)
    {
        $where = "user_id = $this->member_id";
        $order = 'update_time desc';
        $field = 'id,title,click,good,thumbnail,update_time,short_info,tag,status,is_check,hint';
        $tag = "";
        $tags_list = Db::name('tag')->where(array('status' => 1, 'type' => 3))->column('id,name');
    ...
    public function addNovel(Request $request)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data['is_check'] =  (get_config('resource_examine_on')  == 1) ?  0 : 1;
            if($data['is_check'] == 1){
                $reward = get_config('upload_novel_reward');
                if(!empty($reward)){
                    db::name('member')->where(array('id'=>$this->member_id))->setInc('money',$reward);
                }
            }
    ...
    public function agent(Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            if ($user_info['is_agent'] != 1) {
                ...
                //$agent_config = unserialize(preg_replace('!s:(/d+):"(.*?)";!se', '"s:".strlen("$2").":/"$2/";"',$user_info['agent_config']));
                $agent_config = preg_replace_callback('#s:(/d+):"(.*?)";#s',function($match){return 's:'.strlen($match[2]).':"'.$match[2].'";';},$user_info['agent_config']);
                $agent_config = unserialize($agent_config);
    ...
    public function record_video(Request $request)
    {
        ...
        $result = Db::view('video_watch_log', 'id,user_ip,gold,user_id,view_time')
            ...
            ->paginate(15);
        $pages = $result->render();
        $total = Db::name('video_watch_log')->where(['user_id'=>$user_id])->sum('gold');
        $day = Db::name('video_watch_log')->where(['user_id'=>$user_id])->whereTime('view_time', 'today')->sum('gold');
        $info = get_member_info($this->member_id);
        $info['money'] = number_format($info['money'], 2);
        $total_data['total']  = number_format($total, 2);
        $total_data['day']  = number_format( $day , 2);
        $this->assign('total_data',$total_data);
        $this->assign('member_info',$info);
        $this->assign('page_title', '课程消费记录');
        $this->assign('navTopTitle', '课程消费记录');
        $this->assign('result', $result);
        $this->assign('pages', $pages);
    ...
    public function record_img(Request $request)
    {
        ...
        $result = Db::view('atlas_watch_log', 'id,user_ip,gold,user_id,view_time')
            ...
            ->paginate(15);
        $pages = $result->render();
        $total = Db::name('atlas_watch_log')->where(['user_id'=>$user_id])->sum('gold');
        $day = Db::name('atlas_watch_log')->where(['user_id'=>$user_id])->whereTime('view_time', 'today')->sum('gold');
        $info = get_member_info($this->member_id);
        $info['money'] = number_format($info['money'], 2);
        $total_data['total']  = number_format($total, 2);
        $total_data['day']  = number_format( $day , 2);
    ...
    public function record_novel(Request $request)
    {
        ...
        $result = Db::view('novel_watch_log', 'id,user_ip,gold,user_id,view_time')
            ...
            ->paginate(15);
        $pages = $result->render();
        $total = Db::name('novel_watch_log')->where(['user_id'=>$user_id])->sum('gold');
        $day = Db::name('novel_watch_log')->where(['user_id'=>$user_id])->whereTime('view_time', 'today')->sum('gold');
        $info = get_member_info($this->member_id);
        $info['money'] = number_format($info['money'], 2);
        $total_data['total']  = number_format($total, 2);
        $total_data['day']  = number_format( $day , 2);
    ...
    public function record_pay(Request $request)
    {
        ...
        $total = Db::name('order')->where(['user_id'=>$user_id,'status'=>1])->sum('price');
        $day = Db::name('order')->where(['user_id'=>$user_id,'status'=>1])->whereTime('add_time', 'today')->sum('price');
        $info = get_member_info($this->member_id);
        $info['money'] = number_format($info['money'], 2);
        $total_data['total']  = number_format($total, 2);
        $total_data['day']  = number_format( $day , 2);
    ...
    public function get_out_pay(Request $request)
    {
        ...
        //是否开启K币
        $this->assign('config',get_config('is_money_kmoney'));
        return view();
    }
    //会员中心、提现记录
    ...
    //会员中心、K币记录
    public function record_k_gold()
    {
        $user_id=$this->member_id;
        //K币记录
        $list=Db::name('gold_log_k')->where(['user_id'=>$user_id])->order('add_time','desc')->paginate(20, false);
        //提现总记录
        $total_data['total_tixian'] = Db::name('gold_log_k')
            ->where(array('type'=>1,'user_id'=>$user_id))->sum('gold');
        $total_data['total_tixian'] = abs($total_data['total_tixian']);
        //分成总记录
        $total_data['total_commission'] = Db::name('gold_log_k')
            ->where(array('type'=>0,'user_id'=>$user_id))->sum('gold');
        //个人信息
        $info = get_member_info($this->member_id);
        //数据分页
        $pages=$list->render();
        ...
        $this->assign('list',$list);
        $this->assign('pages',$pages);
        $this->assign('current_left_menu', 'record_k_gold');
        return view();
    }
    //会员中心、金币记录
    ...
    {
        $user_id=$this->member_id;
        $list=Db::name('gold_log')->where(['user_id'=>$user_id])->order('add_time','desc')->paginate(20, false);

        $today = strtotime(date('Y-m-d', time()));
        $total_income_where = 'gold > 0 and user_id = '.$user_id;
        $total_consume_where = 'gold < 0 and user_id = '.$user_id;
        $income_where = $total_income_where.' and add_time > '.$today;
        $consume_where = $total_consume_where.' and add_time > '.$today;
        $total_data['total_income'] = Db::name('gold_log')->where($total_income_where)->sum('gold');
        $total_data['income'] = Db::name('gold_log')->where($income_where)->sum('gold');
        $total_data['total_consume'] = Db::name('gold_log')->where($total_consume_where)->sum('gold');
        $total_data['consume'] = Db::name('gold_log')->where($consume_where)->sum('gold');
        foreach ($total_data as $k=>$v){
            $total_data[$k] = number_format($v, 2);
        }
        $total_data['total_consume'] = ((int)$total_data['total_consume'] < 0 ) ? substr($total_data['total_consume'],1) : $total_data['total_consume'];
        $total_data['consume'] = ((int)$total_data['consume'] < 0 ) ? substr($total_data['consume'],1) : $total_data['consume'];
        $info = get_member_info($this->member_id);
        $info['money'] = number_format($info['money'], 2);
        $pages=$list->render();
        $this->assign('total_data',$total_data);
